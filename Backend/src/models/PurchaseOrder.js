...
// Purchase Order Schema (Unified for Purchase Orders, Purchase Quotations, and Proforma Purchases)
const purchaseOrderSchema = new mongoose.Schema(
  {
    ...
    // Sales order generation tracking
    ...
    status: {
      ...
      enum: [
        ...
        "sent", // Sent to supplier/company (auto-generated or manual)
        "accepted", // Supplier accepted (quotation) OR Order confirmed (purchase_order)
        "rejected", // Supplier rejected OR Order cancelled
        ...
        "converted", // Quotation converted to purchase order
        "cancelled", // Cancelled at any stage
        "expired", // Validity expired (quotations)
        "on_hold", // Temporarily on hold
        "partially_received", // For backward compatibility
      ],
...
purchaseOrderSchema.index({companyId: 1, orderType: 1});
...
purchaseOrderSchema.index({hasGeneratedSalesOrder: 1});
...
purchaseOrderSchema.virtual("isBidirectionalOrder").get(function () {
  return !!(this.sourceCompanyId && this.targetCompanyId);
});

purchaseOrderSchema.virtual("hasSalesOrderGenerated").get(function () {
  return !!(this.autoGeneratedSalesOrder && this.salesOrderRef);
});

// ✅ UNIFIED ORDER TYPE VIRTUALS
purchaseOrderSchema.virtual("isPurchaseOrder").get(function () {
  return this.orderType === "purchase_order";
});

purchaseOrderSchema.virtual("isPurchaseQuotation").get(function () {
  return this.orderType === "purchase_quotation";
});

purchaseOrderSchema.virtual("isProformaPurchase").get(function () {
  return this.orderType === "proforma_purchase";
});

// ✅ UNIFIED STATUS CHECKING VIRTUALS
purchaseOrderSchema.virtual("isDraft").get(function () {
  return this.status === "draft";
});

purchaseOrderSchema.virtual("isSent").get(function () {
  return this.status === "sent";
});

purchaseOrderSchema.virtual("isAccepted").get(function () {
  return this.status === "accepted";
});

purchaseOrderSchema.virtual("isConfirmed").get(function () {
  return this.status === "confirmed";
});

purchaseOrderSchema.virtual("isCompleted").get(function () {
  return this.status === "completed";
});

purchaseOrderSchema.virtual("isCancelled").get(function () {
  return this.status === "cancelled";
});

purchaseOrderSchema.virtual("isExpiredStatus").get(function () {
  return (
    this.status === "expired" ||
...
purchaseOrderSchema.virtual("canBeAccepted").get(function () {
  return this.status === "sent";
});

purchaseOrderSchema.virtual("canBeConfirmed").get(function () {
  return ["accepted", "sent"].includes(this.status);
});

purchaseOrderSchema.virtual("canBeProcessed").get(function () {
  return ["confirmed", "approved"].includes(this.status);
});

purchaseOrderSchema.virtual("canBeShipped").get(function () {
  return ["processing", "approved"].includes(this.status);
});

purchaseOrderSchema.virtual("canBeDelivered").get(function () {
  return this.status === "shipped";
});

purchaseOrderSchema.virtual("canBeReceived").get(function () {
  return this.status === "delivered";
});

purchaseOrderSchema.virtual("canBeCompleted").get(function () {
  return this.status === "received";
});

purchaseOrderSchema.virtual("canBeConvertedToInvoice").get(function () {
  return (
    this.orderType === "purchase_order" &&
    ["confirmed", "approved", "processing", "received"].includes(this.status)
  );
});

purchaseOrderSchema.virtual("canBeConvertedToPurchaseOrder").get(function () {
  return this.orderType === "purchase_quotation" && this.status === "accepted";
});

purchaseOrderSchema.virtual("needsConfirmation").get(function () {
...
purchaseOrderSchema.pre("save", function (next) {
  ...
  if (this.isNew && (!this.status || this.status === null)) {
    // Auto-generated orders (from sales orders)
    if (this.isAutoGenerated && this.generatedFrom === "sales_order") {
      this.status = "sent"; // Sent to supplier for confirmation
      console.log(`✅ Auto-generated ${this.orderType} → Status: sent`);
    }

    // Converted orders (from quotations to purchase orders)
    else if (
      this.sourceOrderType === "purchase_quotation" &&
      this.sourceOrderId
    ) {
    ...
    // Bidirectional orders (between companies)
      ...
      this.sourceOrderType === "purchase_order" &&
  ...
  if (this.isNew) {
    ...
    if (this.autoGeneratedSalesOrder && this.salesOrderNumber) {
      trackingNotes.push(`Generated sales order ${this.salesOrderNumber}`);
    }
...
purchaseOrderSchema.methods.acceptOrder = function (acceptedBy = "system") {
  if (this.orderType === "purchase_quotation") {
    this.status = "accepted"; // Quotation accepted
    console.log(`✅ Purchase quotation ${this.orderNumber} accepted`);
  } else {
    this.status = "confirmed"; // Purchase order confirmed
    console.log(`✅ Purchase order ${this.orderNumber} confirmed`);
  }
...
purchaseOrderSchema.methods.rejectOrder = function (
  rejectedBy = "system",
  reason = ""
) {
...
purchaseOrderSchema.methods.confirmOrder = function (confirmedBy = "system") {
  this.status = "confirmed";
  this.lastModifiedBy = confirmedBy;

  // Update corresponding sales order if it exists
  if (this.correspondingSalesOrderId) {
    const SalesOrder = mongoose.model("SalesOrder");
    SalesOrder.findByIdAndUpdate(this.correspondingSalesOrderId, {
      status: "confirmed",
      lastModifiedBy: confirmedBy,
    }).catch((error) => {
      console.error("Error updating corresponding sales order:", error);
    });
  }
...
purchaseOrderSchema.methods.approveOrder = function (approvedBy = "system") {
...
purchaseOrderSchema.methods.startProcessing = function (
  processedBy = "system"
) {
...
purchaseOrderSchema.methods.shipOrder = function (shippedBy = "system") {
  this.status = "shipped";
...
purchaseOrderSchema.methods.deliverOrder = function (deliveredBy = "system") {
...
// Receive Order (unified method)
purchaseOrderSchema.methods.receiveOrder = function (receivedBy = "system") {
  this.status = "received";
  this.deliveryDate = new Date();
  this.lastModifiedBy = receivedBy;

  console.log(`✅ ${this.orderType} ${this.orderNumber} received`);
  return this.save();
};

// Complete Order (unified method)
purchaseOrderSchema.methods.completeOrder = function (completedBy = "system") {
...
purchaseOrderSchema.methods.cancelOrder = function (
  cancelledBy = "system",
  reason = ""
) {
...
// Convert Purchase Quotation to Purchase Order (unified method)
purchaseOrderSchema.methods.convertToPurchaseOrder = async function (
  convertedBy = "system"
) {
  if (this.orderType !== "purchase_quotation") {
    throw new Error("Can only convert purchase quotations to purchase orders");
  }

  if (this.status !== "accepted") {
    throw new Error(
      "Can only convert accepted purchase quotations to purchase orders"
    );
  }

  try {
    // Create new purchase order from this quotation
    const purchaseOrderData = {
      ...
      sourceOrderType: "purchase_quotation",
      sourceCompanyId: this.companyId,
      convertedBy,
      convertedAt: new Date(),
      notes: this.notes
        ? `${this.notes} | Converted from purchase quotation ${this.orderNumber}`
        : `Converted from purchase quotation ${this.orderNumber}`,
    };

    const purchaseOrder = new this.constructor(purchaseOrderData);
    ...
    console.log(
      `✅ Purchase quotation ${this.orderNumber} converted to purchase order ${purchaseOrder.orderNumber}`
    );
    return purchaseOrder;
  } catch (error) {
    throw new Error(
      `Failed to convert purchase quotation to purchase order: ${error.message}`
    );
  }
};
...
// Mark Sales Order as Generated (unified method)
purchaseOrderSchema.methods.markSalesOrderGenerated = function (
  salesOrderId,
  salesOrderNumber,
  targetCompanyId,
  linkedCustomerId,
  ...
  this.autoGeneratedSalesOrder = true;
  this.hasGeneratedSalesOrder = true;
  this.salesOrderRef = salesOrderId;
  this.salesOrderNumber = salesOrderNumber;
  this.salesOrderGeneratedAt = new Date();
  this.salesOrderGeneratedBy = generatedBy;
  this.correspondingSalesOrderId = salesOrderId;
  this.correspondingSalesOrderNumber = salesOrderNumber;
...
// Convert to Purchase Invoice (unified method)
purchaseOrderSchema.methods.convertToPurchaseInvoice = async function () {
  if (this.convertedToPurchaseInvoice) {
    throw new Error("Order already converted to purchase invoice");
  }
  ...
  try {
    ...
    console.log("🔄 Creating purchase invoice from order:", {
      orderId: this._id,
    ...
    const invoice = new Purchase(invoiceData);
    await invoice.save();
    ...
    console.log("✅ Order converted to purchase invoice successfully");
    return invoice;
  } catch (error) {
    console.error("❌ Error converting order to purchase invoice:", error);
    throw new Error(
      `Failed to convert order to purchase invoice: ${error.message}`
    );
  }
};
...
purchaseOrderSchema.statics.getOrdersByTypeAndStatus = function (
  companyId,
  orderType,
  status
) {
...
purchaseOrderSchema.statics.getOrdersNeedingConfirmation = function (
  companyId
) {
  return this.find({
    companyId,
...
purchaseOrderSchema.statics.getOrdersByStatus = function (companyId, status) {
  return this.find({
    companyId,
    status,
  })
...
purchaseOrderSchema.statics.getAutoGeneratedOrders = function (
  companyId,
  orderType = null
) {
  const query = {
...
// Get orders with generated sales orders
purchaseOrderSchema.statics.getOrdersWithGeneratedSO = function (
  companyId,
  filters = {}
) {
  const query = {
  ...
  return this.find(query)
    .populate("supplier", "name mobile email")
    .populate("salesOrderRef", "orderNumber status totals")
    .populate("targetCompanyId", "businessName")
    .sort({salesOrderGeneratedAt: -1});
};

// Get bidirectional orders
purchaseOrderSchema.statics.getBidirectionalOrders = function (companyId) {
  return this.find({
    $or: [
...
purchaseOrderSchema.methods.markAsAccepted = function () {
  return this.acceptOrder();
};

purchaseOrderSchema.methods.markAsRejected = function () {
