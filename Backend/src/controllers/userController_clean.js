...
// Get user profile with associated shops
const getUserProfileWithShops = async (req, res) => {
  try {
    const { userId } = req.params;
    
    // Get user profile
    const user = await User.findById(userId).select('-password -passwordResetToken -passwordResetExpires -emailVerificationToken -emailVerificationExpires -twoFactorSecret -backupCodes');
    ...
    // For now, return user without shops since Shop model may not exist
    res.status(200).json({
      success: true,
      data: {
        user,
    ...
    console.error('Error fetching user profile with shops:', error);
    res.status(500).json({
      success: false,
...
const updateUserProfile = async (req, res) => {
  try {
    ...
    ).select('-password -passwordResetToken -passwordResetExpires -emailVerificationToken -emailVerificationExpires -twoFactorSecret -backupCodes');
...
const getUserDashboard = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).select('firstName lastName email businessInfo');
