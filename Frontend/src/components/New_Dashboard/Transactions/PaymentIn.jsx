  ...
  const fetchInvoicesForParty = async (partyId) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/payments/pending-invoices/${partyId}?companyId=${companyId}`,
        {
          method: 'GET',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.selectedInvoice) {
        paymentData.invoiceId = formData.selectedInvoice.value;
      ...
      const response = await fetch(`${API_BASE_URL}/payments/payment-in`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': token ? `Bearer ${token}` : ''
      ...
      if (result.success) {
        ...
        setSavedPaymentData({
          paymentNumber: result.data?.payment?.paymentNumber || 'N/A',
          partyName: formData.party.label.split(' (')[0],
      ...
      <Card className="shadow-sm">
        <Card.Header className="bg-success text-white">
          <div className="d-flex align-items-center">
            ...
            <h5 className="mb-0">Payment In</h5>
          </div>
        </Card.Header>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <svg width="40" height="40" viewBox="0 0 24 24" fill="none">
                  <path d="M20 6L9 17L4 12" stroke="#28a745" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
              <h2 style={{
                color: 'white',
