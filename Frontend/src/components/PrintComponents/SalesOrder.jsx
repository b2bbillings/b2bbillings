...
const SalesOrder = forwardRef(
  ({orderData, onPrint, template: propTemplate}, ref) => {
    ...
    const isQuotation = meta.isQuotation || order.orderType === "quotation";
    const isProformaInvoice =
      meta.isProformaInvoice || order.orderType === "proforma_invoice";
    const isSalesOrder = !isQuotation && !isProformaInvoice;
    ...
    const getDocumentCopy = () => {
      switch (copyType) {
        ...
        default:
          ...
          if (isProformaInvoice) return "PROFORMA FOR BUYER";
          return "SALES ORDER COPY";
      }
    };
        ...
        return (
          <div className="company-logo-placeholder">
            <span className="logo-text">
              {companyName ? companyName.substring(0, 2).toUpperCase() : "SO"}
            </span>
          </div>
        );
      }
        ...
        `}</style>
          ...
          <div className="details-section">
            <div className="customer-details">
              ...
              <p>
                <strong>Customer:</strong> {customer.name || "Customer Name"}
              </p>
              {customer.address && (
                <p>
          ...
          {isQuotation && (
            <div className="quotation-specific">
              <strong>Quotation Notice:</strong> This is a price quotation only.
              No commitment until order confirmation.
              {validityInfo && !validityInfo.isExpired && (
                <span> Valid for {validityInfo.daysRemaining} more days.</span>
              )}
            </div>
          )}
          ...
          {isProformaInvoice && (
            <div className="proforma-specific">
              <strong>Proforma Invoice Notice:</strong> This is not a tax
              invoice. Tax invoice will be issued upon delivery.
              {payment.advanceRequired && (
                <span>
                  {" "}
                  Advance payment of {formatCurrency(
                    payment.advanceRequired
                  )}{" "}
                  required.
                </span>
          ...
          {isSalesOrder && (
            <div className="order-specific">
              <strong>Sales Order Notice:</strong> This is a confirmed order.
              Please proceed with delivery as per terms.
            </div>
          )}
          ...
          {(order.sourceOrderNumber || order.isAutoGenerated) && (
            <div className="source-tracking">
              ...
              {order.sourceOrderNumber && (
                <span>
                  {" "}
                  Generated from Purchase Order: {order.sourceOrderNumber}
                </span>
              )}
          ...
          {order.correspondingPurchaseOrderNumber && (
            <div className="bidirectional-info">
              <strong>🔄 Bidirectional Link:</strong> Linked to Purchase Order:{" "}
              {order.correspondingPurchaseOrderNumber}
            </div>
          )}
          ...
          {(order.termsAndConditions ||
            ...
            <div className="terms-section">
              ...
              {isQuotation && (
                <p>
                  <strong>Quotation Terms:</strong> Prices are subject to change
                  without notice. This quotation is valid for the period
                  mentioned above.
                </p>
              )}
              ...
              {isProformaInvoice && (
                <p>
                  <strong>Proforma Terms:</strong> This is a proforma invoice
                  for customs purposes only. Actual tax invoice will be issued
                  upon delivery.
                </p>
              )}
              ...
              {isSalesOrder && (
                <p>
                  <strong>Order Terms:</strong> This is a confirmed order.
                  Delivery will be made as per the agreed schedule.
                </p>
              )}
            </div>
          )}
