...
import salesOrderService from "../../services/saleOrderService";
...
const SalesOrderManagement = ({
  ...
  const loadSalesOrders = async () => {
    try {
      ...
      if (isAdmin) {
        if (typeof salesOrderService.getAllSalesOrdersForAdmin === "function") {
          response = await salesOrderService.getAllSalesOrdersForAdmin(
            filterParams
          );
        } else {
          response = await salesOrderService.getSalesOrders(
            companyId || "admin",
            {
              ...filterParams,
              isAdmin: true,
      ...
      if (response.success) {
        ...
        const formattedOrders = orders.map((order) => ({
          ...
          customerMobile:
            order.customerMobile || order.customer?.mobile || "N/A",
          ...
          isConverted: order.isConverted || false,
        ...
        throw new Error(response.message || "Failed to load sales orders");
      }
    } catch (error) {
      throw error;
    }
  ...
  const groupOrdersByStatus = (ordersData) => {
    ...
    const grouped = {
      ...
      converted: ordersData.filter(
        (o) => o.isConverted || o.status === "converted"
      ),
      ...
      quotations: ordersData.filter((o) => o.orderType === "quotation"),
      salesOrders: ordersData.filter((o) => o.orderType === "sales_order"),
    };
  ...
  const loadStatistics = async () => {
    try {
      ...
      if (isAdmin) {
        if (
          typeof salesOrderService.getSalesOrderStatsForAdmin === "function"
        ) {
          response = await salesOrderService.getSalesOrderStatsForAdmin();
        } else if (
          typeof salesOrderService.getDashboardSummary === "function"
        ) {
          response = await salesOrderService.getDashboardSummary(
            companyId || "admin"
          );
        } else {
          // Manual calculation from current data
          ...
          const activeCompanies = [
            ...new Set(
              salesOrders
                .map((order) => order.companyId?._id || order.companyId)
                .filter(Boolean)
            ),
          ].length;
          ...
          setDashboardStats({
            ...
            thisMonthAmount: thisMonthOrders.reduce(
              (sum, order) => sum + (order.totalAmount || 0),
              0
            ),
            orderGrowth: 5.2,
        ...
        if (typeof salesOrderService.getDashboardSummary === "function") {
          response = await salesOrderService.getDashboardSummary(companyId);
        }
      }
  ...
  const handleOrderAction = async (action, order) => {
    ...
    switch (action) {
      ...
      case "convert":
        ...
        break;
      case "send":
        await handleStatusUpdate(order, "sent");
        break;
      case "confirm":
        await handleStatusUpdate(order, "confirmed");
        break;
      case "reject":
  ...
  const handleDeleteOrder = async (order) => {
    ...
    try {
      ...
      const response = await salesOrderService.deleteSalesOrder(order.id);
  ...
  const handleDuplicateOrder = async (order) => {
    try {
      ...
      if (typeof salesOrderService.duplicateOrder === "function") {
        response = await salesOrderService.duplicateOrder(order.id, {
  ...
  const handleConvertToInvoice = async (order) => {
    try {
      ...
      const response = await salesOrderService.convertToInvoice(order.id, {
        userId: localStorage.getItem("userId"),
  ...
  const handleStatusUpdate = async (order, newStatus) => {
    try {
      setIsActionLoading(true);
      const response = await salesOrderService.updateOrderStatus(
        order.id,
        newStatus,
        ""
      );
      ...
      if (response.success) {
        addToast?.(`Order ${newStatus} successfully`, "success");
        await loadData();
      } else {
  ...
  const handleExport = async () => {
    try {
      ...
      if (typeof salesOrderService.exportToCSV === "function") {
        const response = await salesOrderService.exportToCSV(
          companyId || "admin",
          filterParams
        );
        if (response.success) {
          addToast?.("Sales orders exported successfully", "success");
        } else {
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      ...
      accepted: {variant: "success", icon: faCheckCircle},
      rejected: {variant: "danger", icon: faTimesCircle},
  ...
  const getOrderTypeBadge = (orderType) => {
    const typeConfig = {
      quotation: {variant: "info", label: "Quotation"},
      sales_order: {variant: "primary", label: "Sales Order"},
      proforma_invoice: {variant: "secondary", label: "Proforma"},
    ...
    return (
      <Badge bg={config.variant} className="d-flex align-items-center gap-1">
        <FontAwesomeIcon icon={faFileText} size="sm" />
        {config.label}
      </Badge>
    );
  };
  ...
  const getConversionStatusBadge = (order) => {
    if (order.isConverted || order.status === "converted") {
      return (
    ...
    return (
      <div className="sales-order-management">
        <div className="text-center py-5">
          ...
          <h5 className="mt-3 text-muted">Loading sales order data...</h5>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0">
            Manage sales orders across all companies ({totalOrders} total
            orders)
          </p>
        </div>
          ...
          <Card className="border-0 shadow-sm text-center h-100">
            <Card.Body>
              ...
              <small className="text-muted">
                {groupedOrders.pending?.length || 0} Pending •{" "}
        ...
        <Col lg={3} md={6}>
          <Card className="border-0 shadow-sm text-center h-100">
            <Card.Body>
              ...
              <small className="text-muted">All sales orders</small>
            </Card.Body>
          </Card>
        </Col>
        <Col lg={3} md={6}>
        ...
        <Card.Body className="p-0">
          {isLoading && (
            <div className="text-center py-3 bg-light">
              ...
              <span className="text-muted">Loading sales orders...</span>
            </div>
          )}
        ...
        <Modal.Body>
          {selectedOrder && (
            <Row>
              ...
              <Col md={6}>
                <h6 className="text-success mb-3">Customer & Amount</h6>
                <p>
