...
// import "./PurchaseInvoiceManagement.css";

function PurchaseInvoiceManagement({adminData, currentUser, addToast}) {
  ...
  // Load purchase invoices for admin
  const loadPurchaseInvoices = async () => {
    try {
      ...
      const response = await purchaseService.getAllPurchasesForAdmin(
        filterParams
      );
      ...
      if (response.success) {
        ...
        const formattedInvoices = invoices.map((invoice) => ({
          ...
          purchaseNumber: invoice.purchaseNumber || invoice.billNumber || "N/A",
          companyName:
            invoice.companyId?.businessName || invoice.companyId?.name || "N/A",
          supplierName: invoice.supplierName || invoice.supplier?.name || "N/A",
          supplierMobile:
            invoice.supplierMobile || invoice.supplier?.mobile || "N/A",
          supplierEmail:
            invoice.supplierEmail || invoice.supplier?.email || "N/A",
          purchaseDate:
            invoice.purchaseDate || invoice.billDate || invoice.createdAt,
        ...
        setPurchases(formattedInvoices);
        ...
        groupPurchasesByStatus(formattedInvoices);
      } else {
        throw new Error(response.message || "Failed to load purchase invoices");
      }
    } catch (error) {
      throw error;
    }
  ...
  // Group purchases by payment status
  const groupPurchasesByStatus = (purchasesData) => {
    const grouped = {
      all: purchasesData,
      paid: purchasesData.filter((p) => p.paymentStatus === "paid"),
      partial: purchasesData.filter((p) => p.paymentStatus === "partial"),
      pending: purchasesData.filter((p) => p.paymentStatus === "pending"),
      overdue: purchasesData.filter((p) => {
        if (p.dueDate && p.pendingAmount > 0) {
          return new Date(p.dueDate) < new Date();
        }
        return false;
      }),
      dueToday: purchasesData.filter((p) => {
        if (p.dueDate && p.pendingAmount > 0) {
  ...
  const loadStatistics = async () => {
    try {
      const [statsResponse, paymentResponse] = await Promise.all([
        purchaseService.getAdminPurchaseDashboardSummary(),
        purchaseService.getPurchaseStatsForAdmin(),
      ]);
  ...
  const handleInvoiceAction = async (action, purchase) => {
    ...
    switch (action) {
      ...
      case "edit":
        ...
        break;
      case "delete":
        await handleDeleteInvoice(purchase);
        break;
      case "payment":
        await handleAddPayment(purchase);
        break;
      case "updateStatus":
        await handleStatusUpdate(purchase);
        break;
      default:
        break;
    }
  ...
  const handleDeleteInvoice = async (purchase) => {
    if (
      !window.confirm(
        `Are you sure you want to delete invoice "${purchase.purchaseNumber}"?`
      )
    ) {
      return;
    }
  ...
  // Handle add payment
  const handleAddPayment = async (purchase) => {
    const paymentAmount = prompt(
      `Enter payment amount (Pending: ₹${purchase.pendingAmount}):`
    );
    if (!paymentAmount || isNaN(paymentAmount)) return;

    try {
      setIsActionLoading(true);
      const response = await purchaseService.addPayment(purchase.id, {
  ...
  const handleStatusUpdate = async (purchase) => {
    ...
    try {
      setIsActionLoading(true);
      const response = await purchaseService.updatePurchaseStatus(
        purchase.id,
        newStatus
      );
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await purchaseService.exportCSV("admin", filterParams);

      if (response.success) {
        addToast?.("Purchase invoices exported successfully", "success");
      } else {
  ...
  const getPaymentStatusBadge = (purchase) => {
    ...
    const pendingAmount = purchase.pendingAmount;
  ...
  const PortalDropdown = ({purchase, isOpen, position}) => {
            ...
            if (dropItem.type === "header") {
              ...
              return (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleInvoiceAction(dropItem.action, purchase);
                  }}
  ...
  const ThreeDotMenu = ({purchase}) => {
    const isDropdownOpen = activeDropdown === purchase.id;

    return (
      <div className="dropdown-container">
        <button
          className="dropdown-trigger three-dot-menu"
          onClick={(e) => handleDropdownToggle(purchase.id, e)}
          disabled={isActionLoading}
        >
  ...
  if (isLoading && purchases.length === 0) {
    return (
      <div className="purchase-invoice-management">
        <div className="text-center py-5">
          <Spinner animation="border" variant="primary" size="lg" />
          <h5 className="mt-3 text-muted">Loading purchase invoice data...</h5>
    ...
    <div className="purchase-invoice-management">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0">
            Manage purchase invoices across all companies ({totalInvoices} total
            invoices)
          </p>
        </div>
          ...
          <Card className="border-0 shadow-sm text-center h-100">
            <Card.Body>
              ...
              <h3 className="text-primary mb-2">
                {dashboardStats.totalPurchases || 0}
              </h3>
              <p className="text-muted mb-0">Total Invoices</p>
              <small className="text-muted">
                {groupedPurchases.pending?.length || 0} Pending •{" "}
                {groupedPurchases.paid?.length || 0} Paid
              </small>
            </Card.Body>
          </Card>
        </Col>
        <Col lg={3} md={6}>
          <Card className="border-0 shadow-sm text-center h-100">
            <Card.Body>
              ...
              <small className="text-muted">All purchase invoices</small>
            </Card.Body>
          </Card>
        </Col>
        <Col lg={3} md={6}>
        ...
        <Col lg={3} md={6}>
          <Card className="border-0 shadow-sm text-center h-100">
            <Card.Body>
              ...
              <small className="text-muted">Using purchase invoices</small>
            </Card.Body>
          </Card>
        </Col>
      </Row>
        ...
        <Card.Body className="p-3">
          <div className="d-flex gap-2 flex-wrap">
            ...
            >
              ...
              All ({groupedPurchases.all?.length || 0})
            </Button>
            <Button
            ...
            >
              ...
              Paid ({groupedPurchases.paid?.length || 0})
            </Button>
            <Button
            ...
            >
              ...
              Partial ({groupedPurchases.partial?.length || 0})
            </Button>
            <Button
              variant={
            ...
            >
              ...
              Pending ({groupedPurchases.pending?.length || 0})
            </Button>
            <Button
            ...
            >
              <FontAwesomeIcon icon={faExclamationTriangle} className="me-2" />
              Overdue ({groupedPurchases.overdue?.length || 0})
            </Button>
            <Button
            ...
            >
              <FontAwesomeIcon icon={faClock} className="me-2" />
              Due Today ({groupedPurchases.dueToday?.length || 0})
            </Button>
          </div>
        </Card.Body>
      </Card>
          ...
          <Row className="align-items-end">
            ...
            <Col lg={3} md={6} className="mb-3">
              ...
              <Form.Select
                ...
                className="form-select-custom"
              >
                <option value="purchaseDate-desc">Latest First</option>
                <option value="purchaseDate-asc">Oldest First</option>
                <option value="totalAmount-desc">Highest Amount</option>
                <option value="totalAmount-asc">Lowest Amount</option>
                <option value="purchaseNumber-asc">Invoice Number A-Z</option>
                <option value="purchaseNumber-desc">Invoice Number Z-A</option>
          ...
          <div className="table-container">
            <div className="table-responsive table-scrollable">
              <Table hover className="mb-0 modern-table">
                ...
                <tbody>
                  {getCurrentData().map((purchase) => (
                    <tr
                      key={purchase.id}
                      className="clickable-row"
                      style={{cursor: "pointer"}}
                      onClick={() => handleInvoiceAction("view", purchase)}
                    >
                      <td className="py-3">
                      ...
                      >
                        <ThreeDotMenu purchase={purchase} />
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      >
          ...
          {selectedPurchase && (
            <Row>
              <Col md={6}>
                ...
                <p>
                  <strong>Company:</strong> {selectedPurchase.companyName}
                </p>
                <p>
                ...
                <p>
                  <strong>Items:</strong> {selectedPurchase.itemCount}
                </p>
              </Col>
              <Col md={6}>
                <h6 className="text-success mb-3">Supplier & Payment</h6>
                <p>
                ...
                {selectedPurchase.supplierMobile &&
                  selectedPurchase.supplierMobile !== "N/A" && (
                    <p>
                      <strong>Mobile:</strong> {selectedPurchase.supplierMobile}
                    </p>
                  )}
                <p>
                ...
                <p>
                  ...
                  {getPaymentStatusBadge(selectedPurchase)}
                </p>
                <p>
                ...
                {selectedPurchase.pendingAmount > 0 && (
                  <p>
                    ...
                    <span className="text-warning">
                      {formatCurrency(selectedPurchase.pendingAmount)}
                    </span>
                  </p>
                )}
              ...
              {selectedPurchase.notes && (
                <Col md={12} className="mt-3">
                  ...
                  <p className="text-muted">{selectedPurchase.notes}</p>
                </Col>
              )}
            </Row>
          )}
        ...
        <Modal.Footer className="bg-light border-0">
          ...
          {selectedPurchase && modalMode === "view" && (
            <>
              <Button
              ...
              {selectedPurchase.pendingAmount > 0 && (
                <Button
                  variant="success"
                  onClick={() =>
                    handleInvoiceAction("payment", selectedPurchase)
                  }
                  className="professional-button"
                >
