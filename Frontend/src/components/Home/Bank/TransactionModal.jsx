...
function TransactionModal({ show, onHide, account, formData, onInputChange, onSaveTransaction }) {
    ...
    const currentConfig = transactionTypeConfig[formData?.transactionType] || transactionTypeConfig['deposit'];
    ...
    const validateForm = () => {
        ...
        if (formData.transactionType === 'adjust-balance' && !formData.description?.trim()) {
            errors.description = 'Please provide a reason for balance adjustment';
        }

        // Transfer validation
        if (formData.transactionType?.includes('transfer') && formData.transactionType !== 'adjust-balance') {
        ...
        <Modal show={show} onHide={onHide} centered size="lg">
            ...
            <Form onSubmit={handleSubmit}>
                <Modal.Body className="p-4">
                        ...
                        <Row className="align-items-center">
                            ...
                            <Col xs="auto">
                                <div className="text-end">
                                    <div className={`h6 mb-0 ${account.currentBalance < 0 ? 'text-danger' : 'text-success'}`}>
                    ...
                    <Alert variant={currentConfig.color} className="d-flex align-items-center">
                        ...
                        <Col md={6}>
                            <Form.Group>
                                <Form.Label className="fw-semibold">
                                    {currentConfig.amountLabel} <span className="text-danger">*</span>
                                </Form.Label>
                                <InputGroup>
                        ...
                        <Col md={12}>
                            <Form.Group>
                                ...
                                <Form.Control
                                    ...
                                    required={formData.transactionType === 'adjust-balance' || 
                                             formData.transactionType?.includes('transfer')}
                                />
                                <Form.Control.Feedback type="invalid">
                        ...
                        <div className="mt-4 p-3 bg-light rounded-3">
                            ...
                            <Row>
                                <Col>
                                    ...
                                    <span className={`fw-bold ${account.currentBalance < 0 ? 'text-danger' : 'text-success'}`}>
                                        ₹{account.currentBalance?.toLocaleString('en-IN') || '0'}
                                    </span>
                                </Col>
                                <Col xs="auto" className="d-flex align-items-center">
                                    <FontAwesomeIcon 
                                        icon={
                                            formData.transactionType === 'deposit' ? faArrowUp :
                                            formData.transactionType === 'withdraw' ? faArrowDown :
                                            faExchangeAlt
                                        }
                                ...
                                <Col>
                                    ...
                                    {(() => {
                                        ...
                                        } else if (formData.transactionType === 'withdraw' || 
                                                 formData.transactionType?.includes('transfer')) {
                                            newBalance = currentBalance - amount;
                                        ...
                                        return (
                                            <span className={`fw-bold ${newBalance < 0 ? 'text-danger' : 'text-success'}`}>
                    ...
                    >
                        {loading ? (
                            ...
                            <>
                                ...
                                {formData.transactionType === 'adjust-balance' ? 'Adjust Balance' : 
                                 formData.transactionType?.includes('transfer') ? 'Process Transfer' :
                                 `${currentConfig.label}`}
                            </>
                        )}
                    </Button>
