...
function PartyDetails({
    ...
    // Frontend/src/components/Home/Party/PartyDetails.jsx - Updated fetchPaymentData function
    ...
    const fetchPaymentData = async () => {
        ...
        try {
            ...
            if (response && response.success) {
                ...
                console.log('✅ PartyDetails: Payment data extracted:', paymentData.length);
                ...
                const processedPayments = paymentData.map(payment => ({
                    ...
                    displayAmount: payment.type === 'payment_in' ? `+₹${payment.amount}` : `-₹${payment.amount}`,
                    displayType: payment.type === 'payment_in' ? 'Payment In' : 'Payment Out',
                    displayDate: new Date(payment.paymentDate || payment.createdAt).toLocaleDateString('en-IN')
                }));

                console.log('🎯 PartyDetails: Processed payments:', processedPayments.length);
                ...
                console.log('⚠️ PartyDetails: No payments found or API error:', response?.message);
                setActualPayments([]);
                if (response?.error) {
    ...
    const { partyTransactions, paymentSummary, allTransactions } = useMemo(() => {
        ...
        const allTransactions = [
            ...
            ...actualPayments.map(payment => ({
                ...
                amount: parseFloat(payment.amount || 0),
                reference: payment.paymentNumber || payment.reference || `PAY-${payment._id?.substring(0, 8)}`,
        ...
        const totalSalesPaidFromHistory = partySales.reduce((sum, sale) => {
            const paid = sale.payments?.reduce((pSum, payment) => {
                return pSum + parseFloat(payment.amount || payment.paidAmount || 0);
            }, 0) || parseFloat(sale.payment?.paidAmount || 0);
            return sum + paid;
        }, 0);

        const totalPurchasesPaidFromHistory = partyPurchases.reduce((sum, purchase) => {
            const paid = purchase.payments?.reduce((pSum, payment) => {
                return pSum + parseFloat(payment.amount || payment.paidAmount || 0);
        ...
        const totalSalesPaid = Math.max(totalSalesPaidFromPayments, totalSalesPaidFromHistory);
        const totalPurchasesPaid = Math.max(totalPurchasesPaidFromPayments, totalPurchasesPaidFromHistory);
        ...
        return {
            partyTransactions: businessTransactions.sort((a, b) => new Date(b.date) - new Date(a.date)),
            paymentSummary: {
                totalSales,
                totalPurchases,
                totalSalesPaid,
    ...
    const getPartyTypeIcon = () => {
        return party?.partyType === 'supplier' || party?.partyType === 'vendor' ? faBuilding : faUserCheck;
    };
                                ...
                                <div>
                                    ...
                                    <div className="text-muted">
                                        ...
                                        <small className="ms-2">
                                            Added on {new Date(party.createdAt || Date.now()).toLocaleDateString('en-IN')}
                                        </small>
                                    </div>
                                </div>
                            </div>
