  ...
  const getPaymentStatusBadge = (sale) => {
    const totalPaid =
      sale.payments?.reduce(
        (sum, payment) => sum + safeToNumber(payment.amount),
        0
      ) ||
            ...
            <tbody>
              {filteredSales.length > 0 ? (
                filteredSales.map((sale) => {
                  ...
                  const saleTotal = safeToNumber(
                    sale.total || sale.finalTotal || 0
                  );
                  const gstAmount = safeToNumber(
                    sale.gstAmount || sale.taxAmount || 0
                  );
                  ...
                  return (
                    <tr key={sale.id}>
                      ...
                      <td>
                        <div>
                          {new Date(
                            sale.invoiceDate || sale.createdAt
                          ).toLocaleDateString()}
                        </div>
                        <small className="text-muted">
                          {new Date(
                      ...
                      <td>
                        ...
                        {sale.installments && sale.installments.length > 0 && (
                          <small className="text-info d-block">
                            {sale.installments.length} installment
                      ...
                      <td className="text-center">
                        <div className="d-flex gap-1 justify-content-center">
                          <Button
                            variant={
                              progress < 100 ? "success" : "outline-success"
                            }
                            size="sm"
              ...
              <div className="col-md-3">
                <div className="fw-bold text-success fs-5">
                  ...
                  {filteredSales
                    .reduce(
                      (sum, sale) =>
                        sum + safeToNumber(sale.total || sale.finalTotal || 0),
                      0
                    )
                    .toLocaleString()}
                </div>
