...
function SalesFormHeader({
    ...
    // Determine if this is supplier mode
    const isSupplierMode = mode === 'purchases' || documentType === 'purchase';
    ...
    // States for party/customer management
    ...
    const loadParties = async (searchTerm = '') => {
        ...
        try {
            ...
            if (response.success) {
                ...
                setParties([]);
                addToast?.(`Failed to load ${entityType}s: ` + (response.message || 'Unknown error'), 'error');
            }
        } catch (error) {
            setParties([]);
    ...
    useEffect(() => {
        ...
        if (partySearchTerm.length >= 2 && !isSelectingPartyRef.current && !formData.customer) {
    ...
    const generateDocumentNumber = (invoiceType = 'non-gst') => {
        ...
        if (isQuotationsMode) {
            const companyPrefix = currentCompany?.code || 'QUO';
            if (invoiceType === 'gst') {
    ...
    const handleInputChange = (field, value) => {
        ...
        if (field === 'gstEnabled' || field === 'invoiceType') {
            ...
            const newInvoiceType = field === 'gstEnabled' ? (value ? 'gst' : 'non-gst') : value;
    ...
    const handlePartyCreated = (newParty) => {
        ...
        addToast?.(`${EntityTypeCapitalized} "${formattedParty.name}" created and selected successfully!`, 'success');
    ...
    return (
        <div className="sales-form-header">
            {/* Error Alert */}
            {searchError && (
                <Alert variant="warning" className="py-2 mb-3" dismissible onClose={() => setSearchError(null)}>
            ...
            <Row className="mb-3">
                ...
                <Col md={6}>
                    ...
                    <Form.Group className="mb-3">
                        ...
                        <Form.Text className="text-info" style={{ fontSize: '12px' }}>
                            {formData.gstEnabled ? '✅ GST will be calculated' : '⚠️ GST will not be applied'}
                        </Form.Text>
                    </Form.Group>
                    ...
                    <Form.Group className="position-relative mb-3">
                        ...
                        {showPartySuggestions && partySearchTerm.length >= 2 && !formData.customer && (
                            <div
                            ...
                            >
                                {filteredParties.length > 0 && (
                                    filteredParties.slice(0, 5).map((party, index) => (
                                        <div
                                            key={party.id}
                                            className={`p-2 border-bottom cursor-pointer ${selectedPartySuggestionIndex === index ? 'bg-primary text-white' : ''}`}
                                            style={{
                                                fontSize: '12px',
                                                cursor: 'pointer',
                                        ...
                                        >
                                            ...
                                            <div className={selectedPartySuggestionIndex === index ? 'text-light' : 'text-muted'} style={{ fontSize: '11px' }}>
                                                ...
                                                {party.companyName && <span className="ms-2">🏢 {party.companyName}</span>}
                                            </div>
                                        </div>
                                    ))
                                )}
                                ...
                                <div
                                    className={`p-2 cursor-pointer bg-light border-top ${selectedPartySuggestionIndex === filteredParties.length ? 'bg-primary text-white' : ''}`}
                                    style={{
                                        fontSize: '12px',
                                        cursor: 'pointer',
                                        transition: 'background-color 0.2s'
                        ...
                        {formData.customer && (
                            <div className="mt-2 p-2 bg-light border-2 rounded" style={{ borderColor: '#28a745' }}>
                                <div className="d-flex justify-content-between align-items-center">
                                    <div>
                                        <div className="fw-bold text-success" style={{ fontSize: '13px' }}>✅ {formData.customer.name}</div>
                                        {formData.customer.mobile && (
                                            <div className="text-muted" style={{ fontSize: '11px' }}>📞 {formData.customer.mobile}</div>
                                        )}
                                        {formData.customer.companyName && (
                                            <div className="text-muted" style={{ fontSize: '11px' }}>🏢 {formData.customer.companyName}</div>
                                        )}
                                    </div>
                                    <button
                                        type="button"
                ...
                <Col md={6}>
                    ...
                    <Form.Group className="mb-3">
                        <Form.Label className="fw-bold text-danger" style={{ fontSize: '14px' }}>
                            {labels.documentNumber || 'Invoice Number'} *
                        </Form.Label>
                        ...
                        <Form.Text className="text-muted" style={{ fontSize: '12px' }}>
                            Format: {formData.gstEnabled ? 'GST-YYYYMMDD-XXXX' : 'INV-YYYYMMDD-XXXX'}
                        </Form.Text>
                    ...
                    <Form.Group className="mb-3">
                        <Form.Label className="fw-bold text-danger" style={{ fontSize: '14px' }}>
                            {labels.documentDate || 'Invoice Date'} *
                        </Form.Label>
                        <div 
