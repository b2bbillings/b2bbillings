...
function OrderFormProductSection({
      ...
      if (response.success) {
        ...
        const formattedProducts = itemList.map((item) => ({
          ...
          purchasePrice: parseFloat(
            item.buyPrice || item.purchasePrice || item.cost || 0
          ),
  ...
  const handleNewItemSaved = (newItem) => {
    ...
    const formattedProduct = {
      ...
      sellingPrice: parseFloat(
        newItem.salePrice || newItem.sellingPrice || newItem.price || 0
      ),
  ...
  const handleModalKeyDown = (e) => {
    ...
    const hasAddNewOption =
      productList.length === 0 && productSearchTerms?.length >= 2;
    ...
    switch (e.key) {
      case "ArrowDown":
        ...
        setSelectedSuggestionIndex((prev) =>
          Math.min((prev ?? -1) + 1, totalOptions - 1)
        );
        break;
      ...
      <div className="order-form-product-section">
                ...
                <div className="d-flex align-items-center text-muted">
                  ...
                  <div>
                    <h5 className="mb-0 text-dark">
                      <strong>
                        {items.filter((item) => item.productName).length}
                      </strong>{" "}
                      {items.filter((item) => item.productName).length === 1
                        ? "Product"
                        : "Products"}{" "}
                      Added
                    </h5>
        ...
        {items.length > 0 && items.some((item) => item.productName) && (
          <Card className="mb-4 border-2" style={{borderColor: "#000"}}>
            <Card.Header
              className="bg-light border-bottom-2"
                        ...
                        <td style={{padding: "12px", fontSize: "13px"}}>
                          <div className="d-flex gap-2">
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                handleEditProduct(items.indexOf(item))
                              }
                              disabled={disabled}
                              title="Edit product"
                            >
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                handleRemoveProduct(items.indexOf(item))
                              }
                              disabled={disabled}
            ...
            <Card.Body>
              <Form.Group>
                ...
                <Form.Control
                  ...
                  onChange={(e) =>
                    onFormDataChange("invoiceDescription", e.target.value)
                  }
                  style={inputStyle}
                  ...
                  <Form.Control
                    ...
                    style={{
                      ...
                      backgroundColor: tempFormData.selectedProduct
                        ? "#e8f5e8"
                        : "white",
                      color: tempFormData.selectedProduct
                        ? "#155724"
                        : "inherit",
                      fontWeight: tempFormData.selectedProduct
                        ? "bold"
                        : "normal",
                    }}
                          ...
                          {products.slice(0, 5).map((product, productIndex) => (
                            <div
                              ...
                              className={`p-2 border-bottom cursor-pointer ${
                                selectedSuggestionIndex === productIndex
                              ...
                              onMouseEnter={() =>
                                setSelectedSuggestionIndex(productIndex)
                              }
                            >
                              <div className="fw-bold">{product.name}</div>
                              <div style={{fontSize: "12px", opacity: 0.8}}>
                                <span className="text-primary fw-bold">
                                  Sale: ₹{product.sellingPrice.toFixed(2)}
                                </span>
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Select
                  ...
                  style={{
                    ...inputStyle,
                    backgroundColor:
                      formData.gstType === "gst" ? "#FFD700" : "#f8f9fa",
          ...
          {tempFormData.selectedProduct && (
            <Alert variant="success" className="mb-3">
              <Row>
                ...
                <Col md={3}>
                  <strong>Stock:</strong> {tempFormData.availableStock}{" "}
                  {tempFormData.unit}
                </Col>
                <Col md={3}>
                  <strong>GST:</strong> ₹
                  {(tempFormData.gstAmount || 0).toFixed(2)}
                </Col>
                <Col md={3}>
                  <strong>Subtotal:</strong> ₹
                  {(tempFormData.subtotal || 0).toFixed(2)}
                </Col>
              </Row>
        ...
        <Modal.Body>
          <Form.Group>
            <Form.Label className="fw-bold">
              Description for:{" "}
              <span className="text-primary">{tempFormData.productName}</span>
            </Form.Label>
            <Form.Control
